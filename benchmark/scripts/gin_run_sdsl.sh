...
LOG_DIR=../log
QUERY_DIR=../res/query
INDEX_DIR=../res/index-sdsl
PERMUTATION_DIR=../res/permutation
CACHE_DIR=../res/cache-sdsl
...
do
  ...
  do
    ...
    INDEX_FILE="$INDEX_DIR/${BASENAME}_index_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_sdsl.gini"
    INDEX_LOG_FILE="$LOG_DIR/index_log_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_sdsl.txt"
    if [[ ! -f $INDEX_FILE ]]; then
      $GIN_DIR/gin index -i "$INPUT_FILE" $PERMUTATION_FLAG "$PERMUTATION_FILE" -o "$INDEX_FILE" -v 2>> "$INDEX_LOG_FILE" &
    fi
    wait
  done
done
  ...
  do
    ...
    if [[ "$CACHE_DEPTH" -ne "0" && $IDX -lt ${#CACHE_DEPTHS[@]} ]]; then
      ...
      do
        for PERMUTATION_DEPTH in "${PERMUTATION_DEPTHS[@]}"
        do
          CACHE_FILE="$CACHE_DIR/${BASENAME}_cache_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_depth_${CACHE_DEPTH}_sdsl.ginc"
          CACHE_LOG_FILE="$LOG_DIR/cache_log_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_depth_${CACHE_DEPTH}_sdsl.txt"
          INDEX_FILE="$INDEX_DIR/${BASENAME}_index_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_sdsl.gini"
      ...
      do
        ...
        do
          ...
          do
            for LENGTH in "${LENGTHS[@]}"
            do
              LOG_FILE="$LOG_DIR/find_log_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_cache_${CACHE_DEPTH}_fork_${FORK}_match_${MATCH}_threads_${THREAD}_length_${LENGTH}${DECODE_SUFFIX}.txt"
