...
# Generate the bipartite graph if it doesn't exist
GRAPH_FILE="$GRAPH_DIR/bipartite_graph_${NUM_VERTICES}.fmdg"
if [[ ! -f $GRAPH_FILE ]]; then
    python3 $GRAPH_SCRIPT "bipartite" $NUM_VERTICES $GRAPH_FILE
fi

# Check if an index file already exists in the INDEX_OUTPUT_DIR, otherwise create one
INDEX_FILE="$INDEX_OUTPUT_DIR/bipartite_graph_${NUM_VERTICES}.fmdi"
...
do
    ...
    touch $LOG_FILE
    echo "[fmd:benchmark] Bipartite graph experiment parameters:" >> $LOG_FILE
