...
if [[ ! -f $INDEX_FILE ]]; then
    ...
    if [[ ! -f $PERMUTATION_FILE ]]; then
        $GIN_DIR/gin permutation -i "$INPUT_FILE" -t "$PERMUTATION_TIME" -u "$PERMUTATION_TIME" -d "$PERMUTATION_DEPTH" -o "$PERMUTATION_FILE" -j $PERMUTATION_NUM_THREADS 2>> "$PERM_LOG_FILE"
    fi
    $GIN_DIR/gin index -i "$INPUT_FILE" -p "$PERMUTATION_FILE" -o "$INDEX_FILE" -s "$SAMPLE_RATE" -r "$SAMPLE_RATE" 2>> "$INDEX_LOG_FILE"
fi

CACHE_FILE="$CACHE_DIR/${BASENAME}_cache_ptime_${PERMUTATION_TIME}_pdepth_${PERMUTATION_DEPTH}_depth_${CACHE_DEPTH}.ginc"
if [[ ! -f $CACHE_FILE ]]; then
    $GIN_DIR/gin query cache -r "$INDEX_FILE" -o "$CACHE_FILE" -j $THREAD -c "$CACHE_DEPTH" 2>> "$CACHE_LOG_FILE"
fi
...
for (( l=1; l<=$DEPTH; l++ )); do
    (perl $QUERY_SCRIPT -l $l -a "$ALPHABET" | $GIN_DIR/gin query find -r "$INDEX_FILE" -C "$CACHE_FILE" -j "$THREAD" $DECODE_FLAG -v 2>> "$LOG_DIR/stats_kmer_length_${l}.txt" | awk '/^[^\t]/ && !/c:0/ {print}' > "$LOG_DIR/kmer_length_$l.txt") &
    running=$(($running + 1))
