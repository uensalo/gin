...
LOG_DIR=../log/query_speed_vs_permutation_depth_variable_time
QUERY_DIR=../res/query/query_speed_vs_permutation_depth_variable_time
INDEX_OUTPUT_DIR=../res/index/query_speed_vs_permutation_depth_variable_time
PERMUTATION_DIR=../res/permutation/query_speed_vs_permutation_depth_variable_time
...
QUERY_FILE="$QUERY_DIR/${BASENAME}_query_speed_vs_permutation_depth_variable_time.fmdq"
...
do
    ...
    TIME=$(echo "l($DEPTH)/l(2)+1" | bc -l)
    TIME=$(echo "$TIME*$BASE_TIME" | bc -l)
    TIME=$(printf "%.0f" $TIME)  # Round up to nearest integer
