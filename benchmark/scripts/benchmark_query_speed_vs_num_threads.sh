...
for NUM_THREADS in "$@"
do
    echo "[fmd:benchmark] Running benchmark for thread count $NUM_THREADS:"
    ...
    $FMD_DIR/fmd query breadth -r $INDEX_FILE -i $QUERY_FILE -j $NUM_THREADS -b $BATCH_SIZE -v 2>> $LOG_FILE
