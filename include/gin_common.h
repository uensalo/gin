 ...
 */
#ifndef GIN_GIN_COMMON_H
#define GIN_GIN_COMMON_H
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <stdbool.h>
...
#define GIN_MAX_ALPHABET_SIZE 256
...
static gin_fstruct_t prm_fstruct = {prm_comp_f, prm_hash_f, prm_free, prm_copy};

// swap utility
static inline void gin_swap(void **a, void **b) {
    void *temp = *a;
    *a = *b;
    *b = temp;
}

static int64_t gin_popcount64(uint64_t x) {
#ifdef GIN_BUILTIN_POPCOUNT
...
static int64_t gin_ceil_log2(uint64_t x) {
...
static bool gin_test_miller_rabin(int_t n, int_t d) {
    int_t a = 2 + rand() % (n - 4);
    int_t x = 1;
    int_t temp = d;
    while (temp > 0) {
...
static bool gin_is_prime(int_t n) {
...
static int_t gin_next_prime_after(int_t n) {
    if (n <= 1) {
        return 2;
    }
    int_t prime = n % 2 == 0 ? n + 1 : n + 2;
    while (!gin_is_prime(prime)) {
        prime += 2;
    }
    return prime;
}
