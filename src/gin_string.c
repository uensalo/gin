...
void gin_string_init(gin_string_t **str, int_t size) {
    gin_string_t *s = calloc(1, sizeof(gin_string_t));
    if(!s) return;
...
void gin_string_append(gin_string_t *str, char_t c) {
    if(str->size == str->capacity) {
        ...
        memset(str->seq + str->size, GIN_STRING_TERMINATOR, (str->size+1) * sizeof(char_t));
    }
    str->seq[str->size++] = c;
}
...
void gin_string_delete(gin_string_t *str, int_t pos) {
    ...
    str->size--;
    str->seq[str->size] = GIN_STRING_TERMINATOR;
}
...
void gin_string_substring(gin_string_t *s, int_t start, int_t end, gin_string_t **subs) {
    ...
    gin_string_init(subs, substring_length);
...
#define VAL(m, i ,j, n) (m)[(i) * (n) + (j)]
    ...
    gin_edit_op_t *cigar = calloc(n+m, sizeof(gin_edit_op_t));
...
void gin_string_phase(gin_string_t *str1, gin_string_t *str2, gin_edit_op_t *edits, int_t edits_len, gin_string_t **p1, gin_string_t **p2) {
    gin_string_t *s1,*s2;
    gin_string_init(&s1, edits_len+1);
    gin_string_init(&s2, edits_len+1);
...
void gin_string_phase_cigar(gin_string_t *str1, gin_string_t *str2, gin_string_t *cigar, gin_string_t **p1, gin_string_t **p2) {
    gin_string_t *s1,*s2;
    gin_string_init(&s1, cigar->size+1);
    gin_string_init(&s2, cigar->size+1);
...
gin_string_t *gin_string_copy(gin_string_t *str) {
    gin_string_t *copy = calloc(1, sizeof(gin_string_t));
    if(!copy) return NULL;
...
void gin_string_concat(gin_string_t **concat, gin_string_t *s1, gin_string_t *s2) {
    ...
    gin_string_init(concat, total_size);
    ...
    (*concat)->seq[total_size] = GIN_STRING_TERMINATOR;
}
...
void gin_string_concat_mut(gin_string_t *s1, gin_string_t *s2) {
    ...
    s1->size = total_size;
    s1->seq[total_size] = GIN_STRING_TERMINATOR;
}

void gin_string_kmp_lps(gin_string_t *pattern, int_t **lps_rval) {
...
void gin_string_kmp_search(gin_string_t *text, gin_string_t *pattern, int_t *lps_in, int_t **pos_rval, int_t *occ_rval) {
    int_t *lps;
    if(!lps_in) {
        gin_string_kmp_lps(pattern, &lps);
    } else {
        lps = lps_in;
    }
    int_t occ = 0;
