 ...
 *
 * gin_bitstream.c is part of gin
 *
 ...
 */
#include "../include/gin_bitstream.h"

void gin_bs_init(gin_bs_t **bs) {
    gin_bs_t *b = calloc(1, sizeof(gin_bs_t));
    if(!b) {
        *bs = NULL;
        return;
    }
...
gin_bs_t *gin_bs_copy(gin_bs_t *bs) {
    gin_bs_t *cpy;
    gin_bs_init_reserve(&cpy, bs->cap_in_words);
    if(!cpy) return NULL;
...
uint_t gin_bs_hash(gin_bs_t *bs) {
    // fnv1a_64
