...
fmd_string_t *generate_random_string(int length) {
...
int test(int length, int rank_sample_rate, int isa_sample_rate, int sup_print) {
    ...
    fmd_string_t *random_string = generate_random_string(length);
    if(!sup_print) {
    ...
    fmd_fmi_t *fmi;
    fmd_fmi_init_with_sa(&fmi, random_string, sa, rank_sample_rate, isa_sample_rate);
    int passed = 1;

    fmd_fmi_qr_t q;
    q.lo = 0;
    q.hi = fmi->no_chars;
    fmd_vector_t *safmi = fmd_fmi_sa(fmi, &q);
    ...
    fmd_vector_free(safmi);
    fmd_fmi_free(fmi);
    fmd_string_free(random_string);
    return passed;
}
