...
int test_optim(int no_vertices, int no_edges, int minl, int maxl, int rank_sample_rate, int isa_sample_rate, int no_queries, int minq, int maxq, int sup_print, int depth) {
    if(!sup_print) {
        double avg_enc_len = (double)((double)no_vertices * (minl + maxl)/2 + no_vertices * 2 + 1 + (double)no_vertices * fmd_ceil_log2(no_vertices))/1000000.0;
        printf("================================================================================\n");
        printf("TEST: V=%d, E=%d, minl=%d, maxl=%d, B=%d, ISA=%d, AVGL=%lf MBP\n",no_vertices,no_edges,minl,maxl,rank_sample_rate,isa_sample_rate, avg_enc_len);
    ...
    fmd_annealing_t *ann;
    fmd_annealing_configure(&ann, random_graph, constraints, 10000000, 1.5, 0.99999, 10);
    if(!sup_print) {
    ...
    fmd_annealing_iterate_until_end(ann);
    if(!sup_print) {
...
int main() {
    ...
    for (int i = 0; i < no_random_tests; i++) {
        ...
        int result = test_optim(V, E, minl, maxl, B, ISA, noQ, minq, maxq, 0, 6);
    }
    return 0;
}
